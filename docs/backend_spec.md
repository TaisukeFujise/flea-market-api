...
| Vertex AI Multimodal Embedding（`gemini-embedding-2`） | `google.golang.org/genai` | 15 秒 | 503 SERVICE_UNAVAILABLE | 未実装。フィードバック Embedding フェーズで追加予定。出力次元はデフォルト 3072（128〜3072 で選択可）のため実装時に `vector(1408)` → `vector(N)` への DB マイグレーションが必要 |
