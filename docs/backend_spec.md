...
全インフラ（Cloud Run / Cloud SQL / Cloud Storage / Secret Manager）をTerraformで管理。
...
`main` ブランチへのpushで自動ビルド・Cloud Runへのデプロイが走る。
...
`github.com/labstack/echo/v4` を使用。
...
internal/
├── domain/
│   ├── entity/          # エンティティ定義（Product, User, Damage...）
│   └── repository/      # リポジトリインターフェース定義
├── usecase/             # ビジネスロジック
├── handler/
│   └── http/            # HTTPハンドラー（リクエスト/レスポンス変換）
├── infrastructure/
│   ├── persistence/     # リポジトリ実装（GORM + PostgreSQL）
│   └── external/        # 外部API実装（Gemini / Vertex AI / Meshy）
└── middleware/          # 認証・ロギング等
```
...
├── pkg/
│   └── firebase/        # Firebase Admin SDK初期化
...
グループは2つ。`GET /api/products/:id` のように未ログインでも使えるがログイン状態によって挙動が変わるエンドポイントは `authed` に入れず、ハンドラー内で `c.Get("userID")` が nil かどうかを確認して分岐する。
...
4. Context("userID")にfirebase_uidをセット
```

### リソース単位の認可はユースケース層で行う

「出品者のみ」「投稿者のみ」「参加者のみ」などの認可はミドルウェアではなくユースケース/ハンドラー層でチェックする。

理由は2つ：

**責務の分離**：ミドルウェアは認証・ロギング・CORSなど横断的関心事を担う場所。「このユーザーがこのリソースを操作できるか」はビジネスロジックなのでユースケース層が適切。ミドルウェアに入れると専用ミドルウェアが増えて管理しにくくなる。

**二重クエリの回避**：ミドルウェアで認可チェックするにはリソースをDBから取得する必要があり、ハンドラーでも同じリソースを取得するため2回クエリが走る。ユースケース層でまとめて行えば1回で済む。

```
ミドルウェア → 「誰か」の確認（認証）← 全エンドポイント共通
ユースケース → 「その人がこのリソースに触れるか」← エンドポイントごとのビジネスロジック
```
...
| シークレット管理 | APIキー等はSecret Managerで管理。コードにハードコードしない |
...
- 初期化：サービスアカウントキーをSecret Managerから取得
...
| DB_HOST | DBホスト | localhost / Cloud SQLのIP |
| DB_PORT | DBポート | 5432 |
| DB_NAME | DB名 | flea_market |
| DB_USER | DBユーザー | postgres |
| DB_PASSWORD | DBパスワード | Secret Manager経由 |
| FIREBASE_PROJECT_ID | FirebaseプロジェクトID | my-project |
| FIREBASE_CREDENTIALS | サービスアカウントキー（JSON） | Secret Manager経由 |
...
| `github.com/pgvector/pgvector-go` | pgvectorのvector型サポート |
| `github.com/golang-migrate/migrate/v4` | DBマイグレーション管理（ENUM定義等、GORMのAutoMigrateで対応できない箇所） |
