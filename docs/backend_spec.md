...
| サービス | Go パッケージ | タイムアウト | エラー時レスポンス |
|---|---|---|---|
| Vertex AI Gemini（傷検出） | `google.golang.org/genai` | 120 秒（goroutine 内） | WebSocket `damage_detection_failed` |
| Vertex AI Multimodal Embedding | `cloud.google.com/go/aiplatform/apiv1beta1` | 15 秒 | 503 SERVICE_UNAVAILABLE |
...
| `github.com/disintegration/imaging` | 画像リサイズ（1024×1024 正規化） |
| `google.golang.org/genai` | Vertex AI Gemini クライアント（傷検出） |
