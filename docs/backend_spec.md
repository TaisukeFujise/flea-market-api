...
| Vertex AI Multimodal Embedding（`gemini-embedding-2-preview`） | `google.golang.org/genai` | 15 秒 | 503 SERVICE_UNAVAILABLE | 凍結中。傷報告時は bbox クロップ画像（GCS URI）を送信して `feedback_embeddings` に保存。傷検出時は全5方向でクエリして pgvector 類似検索し、上位3件のクロップ画像 + damage_type + description を Gemini プロンプトに few-shot として追加する予定 |
...
| `google.golang.org/genai` | Vertex AI Gemini クライアント（傷検出・Embedding［凍結中］） |
