...
上記に加え末尾に追加：
「## 傷の参考例（別商品です）
過去の同カテゴリ商品で購入者から報告された傷の例です。
同様のパターンを見落とさないよう参考にしてください。damagesには含めないでください。」
+ 繰り返し（上位3件）：
    「傷種別: {damage_type} / 説明: {description}」テキスト + クロップ画像（インライン）
```
...
> pgvector を使った傷報告フィードバックループは現フェーズでは実装しない。`damage_reports` テーブルおよびバックエンド API（`POST /api/orders/:id/damage-reports`）は実装済みだが、フロント画面・Embedding 保存・pgvector 検索は凍結。以下は再開時の実装方針として残す。
...
[2] damage_reports INSERT（Go）← 実装済み
    前提条件をサーバー側でも検証（orders.buyer_id・status確認）
    product_id, user_id, image_id, damage_type, bbox_x1/y1/x2/y2, description を保存
    INSERT 後に goroutine で [3]〜[5] を非同期実行（204 レスポンスには影響しない）

[3] bbox クロップ画像の生成・GCS 保存（Go goroutine）← 凍結
    damage_reports.image_id → product_images から GCS URL + category_id（products 経由）を取得
    → Cloud Storage から元画像をダウンロード
    → bbox 座標（0-1000 正規化）を実ピクセル座標に変換
    → Go 標準ライブラリ（image/jpeg / image/png）で bbox 領域をクロップ
    → クロップ画像を GCS にアップロード（例: feedback-crops/{uuid}.jpg）→ URI 取得
    ※ クロップ理由：AIが見落とした傷の特徴パターンを精密に埋め込むため。フル画像では背景ノイズが類似検索の精度を下げる
    ※ GCS 保存理由：検索時に再クロップ不要になりレイテンシを削減する

[4] Embedding 生成（Go → Vertex AI）← 凍結
    クロップ画像の GCS URI を Vertex AI Multimodal Embedding（gemini-embedding-2-preview・3072次元）に送信

[5] feedback_embeddings INSERT ← 凍結
    - damage_report_id
    - category_id（products から取得）
    - cropped_image_url（GCS URI）
    - embedding vector(3072)

[6] 次回の傷検出に反映 ← 凍結
    同カテゴリの新商品が出品されたとき
    → 全5方向の画像それぞれを Vertex AI Multimodal Embedding でベクトル化（クエリ用・5回）
    → 各ベクトルで feedback_embeddings を pgvector 類似検索（同 category_id・コサイン類似度）
    → 5回分の結果をスコアでソートして damage_report_id 単位で重複排除・上位3件に絞る
    → feedback_embeddings.cropped_image_url を使って GCS からクロップ画像を直接取得（再クロップ不要）
    → クロップ画像 + damage_type + description をプロンプトに追加（参照専用・few-shot）
    ※ フル画像ではなくクロップを渡す理由：背景ノイズを排除し傷パターンだけを Gemini に伝えるため。description で文脈を補う
```
