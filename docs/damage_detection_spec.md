...
[2] 傷検出 + 状態サマリー生成（Go goroutine → Vertex AI Gemini）
    ...
    ※ pgvector フィードバックがある場合（feedback_embeddings に同カテゴリのレコードが存在する場合）
    → 全5方向の画像それぞれを Vertex AI Multimodal Embedding（gemini-embedding-2-preview）でベクトル化（5回）
...
[3] bbox クロップ画像の生成・GCS 保存（Go goroutine）
    ...
    → クロップ画像を GCS にアップロード（例: feedback-crops/{uuid}.jpg）→ URL 取得
    ※ クロップ理由：AIが見落とした傷の特徴パターンを精密に埋め込むため。フル画像では背景ノイズが類似検索の精度を下げる
    ※ GCS 保存理由：検索時に再クロップ不要になりレイテンシを削減する

[4] Embedding 生成（Go → Vertex AI）
    クロップ画像をインライン（バイト列）で Vertex AI Multimodal Embedding（gemini-embedding-2-preview・3072次元）に送信

[5] feedback_embeddings INSERT
    - damage_report_id
    - category_id（products から取得）
    - cropped_image_url（GCS URL）
