...
- `feedback_embeddings.embedding` は現在 `vector(1408)`（旧 `multimodalembedding@001` の次元数）。`gemini-embedding-2` 実装時に次元数を変更するマイグレーションが必要（デフォルト 3072、128〜3072 で選択可）。`category_id` と組み合わせたカテゴリ内類似検索で Gemini への few-shot 参照に使用
