...
| damage_reports | 購入者が受け取り後に報告。AIの精度向上目的のみで出品者には通知されない。`feedback_embeddings` への連携は凍結中 |
...
| feedback_embeddings | pgvector フィードバックループ用（凍結中）。傷報告の bbox クロップ画像を埋め込んで保存し、同カテゴリの類似傷報告を傷検出の few-shot 参照に使う設計。`cropped_image_url` にクロップ画像の GCS URI を保持 |
...
- `feedback_embeddings.embedding` は `vector(3072)`（`gemini-embedding-2-preview` の出力次元数）。`category_id` と組み合わせたカテゴリ内 pgvector 類似検索で Gemini への few-shot 参照に使用する設計（凍結中）
